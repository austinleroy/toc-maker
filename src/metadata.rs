...
type UtocMetadataFlags = u8;
const USE_COMPRESSION_ZLIB  : u8 = 1 << 0;
const USE_COMPRESSION_OODLE : u8 = 1 << 1;
const USE_COMPRESSION_LZ4   : u8 = 1 << 2;
const USE_COMPRESSION_GZIP  : u8 = 1 << 3;
...
impl UtocMetadata {
    ...
    pub fn add_entries<TByteOrder: byteorder::ByteOrder>(&mut self, data: Vec<u8>) {
        ...
        for i in 0..compressed_package_count {
            self.compressed_assets.insert(compressed_assets[i as usize], metadata_reader.read_u8().unwrap()).unwrap();
        }
    }
